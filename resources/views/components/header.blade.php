    ...
    <nav class="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800">
        <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
            ...
            <div class="flex items-center lg:order-2">
                        ...
                        @role('customer')
                            <ul class="p-2" aria-labelledby="dropdown">
                                @if (auth()->user()->sellerRequest && auth()->user()->sellerRequest->status === 'pending')
                                    <span class="block py-2 px-4 text-sm rounded-md text-center text-white bg-green-500 font-semibold">
                ...
                @else
                    <a href="{{ route('login') }}"
                        class="text-gray-800 dark:text-white dark:hover:text-green-400 hover:text-green-600 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 focus:outline-none dark:focus:ring-gray-800">
                        Log in
                    </a>
                    <a href="{{ route('register') }}"
                        class="text-white bg-green-500 hover:bg-green-600 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 dark:bg-green-500 dark:hover:bg-green-600 focus:outline-none dark:focus:ring-green-700">
                        Register
                    </a>
                @endauth
                <x-theme-button />
